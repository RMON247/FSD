...
export default function ReorderListModal({ open, onClose }) {
  ...
  return (
    <Modal
      open={open} onClose={onClose} title="Reorder List" subtitle="Review quantities, then submit as a batch stock-in"
      footer={items.length > 0 && (<><Button variant="secondary" onClick={clear}>Clear List</Button><Button icon={PackageCheck} onClick={submit}>Submit Reorder ({items.length})</Button></>)}
    >
      {items.length === 0 ? (
          ...
          {items.map((i) => (
            <div key={i.id} className="flex items-center justify-between py-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <input type="number" value={i.quantity} onChange={(e) => setQuantity(i.id, Number(e.target.value) || 1)} className="w-14 text-center text-sm tabular rounded-md border border-slate-200 dark:border-ink-600 bg-white dark:bg-ink-900 py-1" />
