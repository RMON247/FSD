...
export default function StockAdjustModal({ open, onClose, product, onSubmit }) {
  ...
  const handleSubmit = () => {
    ...
    if (direction === 'out' && qty > product.quantity) { setError(`Only ${product.quantity} ${product.unit} available to remove`); return }
  ...
  return (
    <Modal
      open={open} onClose={onClose} title="Adjust Stock" subtitle={`${product.name} · ${product.sku}`}
      footer={<><Button variant="secondary" onClick={onClose}>Cancel</Button><Button variant={direction === 'in' ? 'success' : 'primary'} onClick={handleSubmit}>{direction === 'in' ? 'Confirm Stock In' : 'Confirm Stock Out'}</Button></>}
    >
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => handleDirectionChange('in')} className={`flex items-center justify-center gap-2 rounded-lg border py-2.5 text-sm font-medium transition-colors ${direction === 'in' ? 'border-teal-500 bg-teal-50 text-teal-700 dark:bg-teal-500/10 dark:text-teal-400' : 'border-slate-200 dark:border-ink-600 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-ink-700'}`}>
              <ArrowDownRight size={15} /> Stock In
            </button>
            <button onClick={() => handleDirectionChange('out')} className={`flex items-center justify-center gap-2 rounded-lg border py-2.5 text-sm font-medium transition-colors ${direction === 'out' ? 'border-amber-500 bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' : 'border-slate-200 dark:border-ink-600 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-ink-700'}`}>
              <ArrowUpRight size={15} /> Stock Out
            </button>
          </div>
        </div>
        <TextField label={`Quantity (${product.unit})`} required type="number" min="1" value={quantity} error={error} onChange={(e) => { setQuantity(e.target.value); setError('') }} placeholder="0" />
