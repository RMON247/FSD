...
export function DataProvider({ children }) {
  ...
  const addProduct = useCallback((data) => {
    setProducts((prev) => {
      ...
      logTransaction({
        type: 'Stock In', productId: product.id, productName: product.name, sku: product.sku,
        quantity: product.quantity, reason: 'New Product Added', storageId: product.storageId
      })
      return [product, ...prev]
    })
  }, [logTransaction])
  ...
  const adjustStock = useCallback((productId, direction, quantity, reason) => {
    ...
    setProducts((prev) => prev.map((p) => {
      ...
      return { ...p, quantity: newQty, status: computeStatus(newQty, p.minStock), value: Number((p.price * newQty).toFixed(2)) }
    }))
    logTransaction({
      type: direction === 'in' ? 'Stock In' : 'Stock Out', productId, productName, sku,
      quantity, reason: reason || (direction === 'in' ? 'Manual Restock' : 'Manual Deduction'), storageId
    })
  }, [logTransaction])
  ...
  const addCustomer = useCallback((data) => {
    setCustomers((prev) => {
      ...
      return [{
        id, name: data.name, email: data.email, phone: data.phone, status: data.status || 'Active',
        joinedOn: new Date().toISOString(), totalPurchases: 0, orderCount: 0, purchaseHistory: []
      }, ...prev]
    })
  }, [])
