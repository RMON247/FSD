...
function Toggle({ checked, onChange }) {
  return (
    <button onClick={() => onChange(!checked)} className={`relative h-6 w-11 rounded-full transition-colors shrink-0 ${checked ? 'bg-brand-600' : 'bg-slate-200 dark:bg-ink-600'}`}>
...
export default function Settings() {
  ...
  const [notifications, setNotifications] = useState({ lowStock: true, outOfStock: true, dailySummary: false, newCustomers: true })
  const [profile, setProfile] = useState({ name: 'Maria Winters', email: 'maria.winters@stockyard.com', role: 'Warehouse Manager', warehouse: 'Central Warehouse #04' })
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <Card>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[{ key: 'light', label: 'Light', icon: Sun }, { key: 'dark', label: 'Dark', icon: Moon }, { key: 'system', label: 'System', icon: Monitor }].map((opt) => (
            <button
              key={opt.key}
              onClick={() => { if (opt.key === 'system') setTheme(window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'); else setTheme(opt.key) }}
      ...
      <Card>
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-ink-700 flex justify-end"><Button icon={Save} onClick={handleSaveProfile}>Save Profile</Button></div>
      </Card>
        ...
        <div className="space-y-1">
          {[
            ...
            <div key={item.key} className="flex items-center justify-between py-3 border-b last:border-0 border-slate-100 dark:border-ink-700">
              <div><p className="text-sm font-medium text-slate-700 dark:text-slate-200">{item.label}</p><p className="text-xs text-slate-400">{item.desc}</p></div>
              <Toggle checked={notifications[item.key]} onChange={(v) => setNotifications({ ...notifications, [item.key]: v })} />
            </div>
          ))}
        </div>
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-ink-700 flex justify-end"><Button icon={Save} onClick={handleSaveNotifications}>Save Preferences</Button></div>
      </Card>
